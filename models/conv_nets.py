...
class MaskedGatedConv2d(nn.Module):
    def __init__(self, mask_type, *args, **kwargs):
        ...
        self.output_channels = args[1]
        self.h = MaskedConv2d(mask_type, *args, **kwargs)
        self.bnh = nn.BatchNorm2d(self.output_channels)
        self.g = MaskedConv2d(mask_type, *args, **kwargs)
        self.bng = nn.BatchNorm2d(self.output_channels)

    def forward(self, x, z):
        # previously without BN and tanh
        h = nn.Tanh()(self.bnh(self.h(x) + z))
        g = nn.Sigmoid()(self.bng(self.g(x) + z))
        return h * g